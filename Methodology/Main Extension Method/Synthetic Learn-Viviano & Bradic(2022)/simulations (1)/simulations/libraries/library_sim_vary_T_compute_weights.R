...
helper_sim_weights <- function(y, X, p, N, T_0,  M = Inf, T_train=ceiling(T_0/2),
  ...
  y1 <- y[-c(T_0:N)]
  X1 <- X[-c(T_0:N),]
  mean_Xs = apply(X, 1, mean)
 # demean_X = apply(X, 2, function(x) x - mean_Xs)
 # demeaned_y = y - mean_Xs
...
Simulation_bootstrap_VS_perm_weights <- function(p, N,T_0,  M = Inf, T_train=ceiling(T_0/2),
  ...
  if(is.na(other_N) == F){ 
    ...
    k <- 1
    for(n in other_N){    
      res[k,] <- helper_sim_weights(y[1:n], X[1:n,], p, N = n, T_0 = T_0,  M = Inf, T_train=T_train,
    ...
    helper_sim_weights(y, X, p, N = N, T_0,  M = Inf, T_train=T_train,
               lvl_sign=lvl_sign,RNN=RNN,lasso=lasso, randforest=randforest,fake_experts=fake_experts, strong_experts=strong_experts, 
               weak_experts=weak_experts, fat_tailed_x=fat_tailed_x,
...
final_weights <- function(AR=0.5,p=10, MA=0.3, N_sim = 100, N=100,
                                               lvl_sign=0.05, T_0=90, 
                                               T_train=ceiling(T_0/2), model="ARMA", model_beta="Linear",
  ...
  sample_sizes <- if(is.na(sum(other_N)) == T){N} else {other_N}
    ...
    result <- matrix(NA, nrow=N_sim*rows_number, ncol= fake_experts + 3)
    pass_data <- NA
    for(i in 1:N_sim){
      if(is.na(data) == F){
        pass_data <- data[[i]]
      }
      rr  <- tryCatch(do.call("Simulation_bootstrap_VS_perm_weights", list(lvl_sign = lvl_sign, p=p, N=N,  T_0 = T_0, T_train=T_train,
                                                          alpha=0, AR=AR, MA=MA, model=model, model_beta=model_beta, 
                                                          ...
                                                          lasso_only = lasso_only, 
                                                          data = pass_data)), 
                      error = function(e) return(rep(NA, dim(result)[2])))
      if(rows_number == 1){
        result[i,]  <- rr} else {
  ...
  final_result = apply(result, 2, function(x) mean(x, na.rm = T))
  names(final_result) <- c('XGBoost', 'SVM', 'Arima', paste0('Exp', c(1:fake_experts)))
  return(final_result)}
