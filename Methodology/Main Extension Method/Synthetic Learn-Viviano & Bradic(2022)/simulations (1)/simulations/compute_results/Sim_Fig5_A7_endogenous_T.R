...
helper_sim <- function(y, X, p, N, T_0,  M = Inf, T_train=ceiling(T_0/2),
                       ...
                       model_beta ="Linear",
                       lag=F, is_X_AR=T, sd_Y=1, sparsify=F,size_boot=10, synthetic_control_only=T, my_seed = rnorm(1)){
...
### Exponential weights methods 

## Use only one learner(Synthetic Control method only) 
if(synthetic_control_only){
  weights <- SC_weights(y=y_train - mean(y_train), X=X_train)
  Y_expN <- apply(cbind(X), 1, function(x) weights%*%x) + mean(y_train)
  bootsr <- tsboot(cbind(y[-c(1:T_train)],Y_expN[-c(1:T_train)]), function(x) helper_4boot_simple(x, T_0=T_0 - T_train), R =100, sim="fixed", l=size_boot)
  ...
  bootsr <- tsboot(cbind(y[-c(1:T_train)],Experts_with_fake[-c(1:T_train),]), function(x) helper_4boot_complex(x, T_0=T_0 - T_train), R =100, sim="fixed", l=size_boot)
  quantile <- sort(bootsr$t[,1])[ceiling((1-lvl_sign)*length(bootsr$t[,1]))]
}
...
mean_hat <- mean(y)
weights <- SC_weights(y=y - mean_hat, X=apply(X, 2, function(x) x  -mean(x)))
...
quantile_perm_sc <- sort(test_statistic)[ceiling((1-lvl_sign)*length(test_statistic))]
...
quantile_perm_did <- sort(test_statistic)[ceiling((1-lvl_sign)*length(test_statistic))]
...
Simulation_bootstrap_VS_perm <- function(p, N,T_0,  M = Inf, T_train=ceiling(T_0/2),
                                 ...
                                 weak_experts=F, fat_tailed_x=F,
                                 model_beta ="Linear",
                                 lag=F, is_X_AR=T, sd_Y=1, sparsify=F,size_boot=10, synthetic_control_only=T, other_N = NA){
  ...
  T_0 = round(T_0 + min(max(1 + rexp(1, abs(mean(X))) - 1/abs(mean(X)) , 1), 50))
...
simulation_bootstrap_vs_perm_final <- function(AR=0.5,p=10, MA=0.3, N_sim = 100, N=100,lvl_sign=0.05, T_0=90, 
                                       T_train=ceiling(T_0/2), alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), model="ARMA", model_beta="Linear",
                                      sd_Y=1, fat_tailed_x =F,
                                      lag=F, is_X_AR=T, verbose=T,  size_boot=10, synthetic_control_only=F, other_N = NA, fake_experts=10){
  ...
  for (a in alpha){
    ...
    for(i in 1:N_sim){
      ...
      rr  <- do.call("Simulation_bootstrap_VS_perm", list(lvl_sign = lvl_sign, p=p, N=N,  T_0 = T_0, T_train=T_train,
                                                  alpha=a, AR=AR, MA=MA, model=model, model_beta=model_beta, 
                                                  fat_tailed_x=fat_tailed_x, sd_Y=sd_Y,
                                                        lag=lag, is_X_AR=is_X_AR, size_boot=size_boot, synthetic_control_only = synthetic_control_only, other_N=other_N, fake_experts = fake_experts))
     if(rows_number == 1){
      result[i,]  <- rr} else {
...
simulation2 <- simulation_bootstrap_vs_perm_final(p=50, model="Linear_FM", N = 500, T_0=250, alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), N_sim=500, fake_experts = 50, other_N = c(400))
save.image(file=paste0(working_dir, '/saved_results/boot_perm2_endotime.RData'))
simulation3 <- simulation_bootstrap_vs_perm_final(p=50, model_beta="logit", N = 500, T_0=250, alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), sd_Y=0.1, N_sim=500, fake_experts = 50, other_N = c(400))
save.image(file=paste0(working_dir, '/saved_results/boot_perm2_endotime.RData'))
print('first set done')
simulation4 <- simulation_bootstrap_vs_perm_final(p=50, model_beta="logit", N = 500, T_0=250, alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), sd_Y=1, N_sim=500, fake_experts = 50, other_N = c(400))
save.image(file=paste0(working_dir, '/saved_results/boot_perm2_endotime.RData'))
simulation5 <- simulation_bootstrap_vs_perm_final(p=50, model_beta="Cos", N = 500, T_0=250, alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), sd_Y=0.1, fake_experts = 50, N_sim=500, other_N = c(400))
save.image(file=paste0(working_dir, '/saved_results/boot_perm2_endotime.RData'))
simulation6 <- simulation_bootstrap_vs_perm_final(p=50, model_beta="Cos", N = 500, T_0=250, alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), sd_Y=1, fake_experts = 50, N_sim=500, other_N = c(400))
save.image(file=paste0(working_dir, '/saved_results/boot_perm2_endotime.RData'))
simulation7 <- simulation_bootstrap_vs_perm_final(p=50, model="Factor_model", N = 500, T_0=250, alpha=c(0,0.1, 0.2,0.3,0.5,0.7,1,1.5), N_sim=500, fake_experts = 50, other_N = c(400))
save.image(file=paste0(working_dir, '/saved_results/boot_perm2_endotime.RData'))
