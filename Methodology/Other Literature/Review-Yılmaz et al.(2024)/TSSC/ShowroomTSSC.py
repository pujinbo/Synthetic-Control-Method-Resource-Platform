...
model2 = TSSC(df=df, treat=treat, time=time,
       outcome=outcome,
       unitid=unitid,
       figsize=(10, 8),
       treated_color="black",
...
plt.plot(SC, label='Convex Brooklyn',
         linestyle='--', color='#7DF9FF', linewidth=2)

plt.plot(FDID_unit, label='FDID Brooklyn',
         linestyle='-', color='blue', linewidth=2.5, alpha=.8)
plt.plot(MSCB, label='MSC Brooklyn',
